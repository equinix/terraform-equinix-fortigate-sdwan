...
variable "admin_password" {
  ...
  type        = string
  validation {
    condition     = length(var.admin_password) >= 6 && length(var.admin_password) <= 128
    error_message = "Admin password key has to be from 6 to 128 characters long."
  }
}

variable "controller_ip_address" {
  description = "SD-WAN controller IP address"
  type        = string
  validation {
    condition     = length(var.controller_ip_address) > 0
    error_message = "Controller IP address has to be an non empty string."
  }
}
...
variable "secondary" {
  ...
  validation {
    condition     = !try(var.secondary.enabled, false) || try(length(var.secondary.admin_password) >= 6 && length(var.secondary.admin_password) <= 128, false)
    error_message = "Key 'admin_password' has to be defined for secondary device. Valid admin password  has to be from 6 to 128 characters long."
  }
  validation {
    condition     = !try(var.secondary.enabled, false) || try(length(var.secondary.controller_ip_address) > 0, false)
    error_message = "Key 'controller_ip_address' has to be defined for secondary device. Valid controller IP address has to be an non empty string."
  }
}
